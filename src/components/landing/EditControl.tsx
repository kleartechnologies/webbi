    ...
    <Band z={5} className="bg-surface text-ink">
      <Inner className="grid grid-cols-[repeat(auto-fit,minmax(300px,1fr))] items-center gap-[clamp(28px,4vw,56px)]">
        <div className="lp-reveal flex flex-col gap-[18px]">
          <Eyebrow className="text-blue">Yours to change</Eyebrow>
          <Heading className="text-[clamp(32px,4.8vw,56px)]">Webbi does the hard work. You stay in control.</Heading>
          <ul className="flex flex-col gap-3 text-[16px] leading-[1.5]">
            {LINES.map(([icon, text]) => (
              <li key={icon} className="flex items-start gap-3">
                <Icon name={icon} size={22} className="shrink-0 text-blue" />
                {text}
              </li>
            ))}
          </ul>
        ...
        <div className="lp-reveal" style={vars({ "--lp-delay": "120ms", "--lp-rise": "20px" })}>
          <div data-float="0.06" aria-hidden className="flex flex-col gap-[14px] rounded-[28px] border border-line bg-ground p-5 [transform:translateY(var(--py,0px))]">
            <span className="text-[11px] font-bold uppercase tracking-[0.08em] text-muted">Style</span>
            ...
            <span className="text-[11px] font-bold uppercase tracking-[0.08em] text-muted">Accent colour</span>
            ...
            <span className="text-[11px] font-bold uppercase tracking-[0.08em] text-muted">Button wording</span>
            <div className="flex flex-wrap gap-2">
              <span className="flex h-9 items-center gap-[6px] rounded-pill bg-whatsapp px-[14px] text-[13px] font-bold text-white">
                <Icon name="chat" size={16} fill />
                Order on WhatsApp
              </span>
              <span className="flex h-9 items-center rounded-pill border-[1.5px] border-line-input bg-surface px-[14px] text-[13px] font-bold text-ink">Book a table</span>
              <span className="flex h-9 items-center rounded-pill border-[1.5px] border-line-input bg-surface px-[14px] text-[13px] font-bold text-ink">Call us</span>
            </div>
          </div>
        </div>
      </Inner>
