 ...
 * what the visitor is holding. The sites themselves are customer content and
 * do not follow the landing's language.
 */
...
function Card({ b }: { b: ShowcaseBusiness }) {
  ...
  const button = showcase.buttons[b.category as keyof typeof showcase.buttons] ?? category.cta;
      ...
      <article aria-labelledby={id} className="group flex h-full flex-col gap-3">
        <header className="flex items-start justify-between gap-3">
          <div className="flex min-w-0 flex-col gap-[2px]">
            ...
            <p className="text-[13px] text-white/60">{showcase.subs[b.slug]}</p>
          </div>
        ...
        <footer className="mt-1 flex min-h-6 items-center justify-between gap-3 text-[12.5px]">
          <span className="inline-flex min-w-0 items-center gap-[6px] text-white/60">
            <Icon name={category.ctaIcon} size={15} className="shrink-0 text-sun" />
            {button}
          </span>
          {isLive(b) ? (
            <Link
              ...
              target="_blank"
              rel="noopener"
              aria-label={showcase.openLabel(b.name)}
              className="inline-flex shrink-0 items-center gap-1 font-semibold whitespace-nowrap text-white/80 transition-colors hover:text-white"
            >
              {showcase.open}
    ...
    <div className="mt-[clamp(56px,7vw,104px)] flex flex-col gap-[clamp(22px,3vw,40px)]">
      <div className="lp-reveal mx-auto flex w-full max-w-[1180px] flex-wrap items-end justify-between gap-x-10 gap-y-3 px-4">
        <div className="flex max-w-[640px] flex-col gap-2">
          <h3 className="font-display text-[clamp(24px,3vw,36px)] font-extrabold leading-[1.08] tracking-[-0.03em] text-balance">{showcase.title}</h3>
